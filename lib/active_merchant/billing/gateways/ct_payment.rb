  ...
  module Billing #:nodoc:
    class CtPaymentGateway < Gateway
      ...
      def purchase(money, payment, options={})
        ...
        response = payment.is_a?(String) ? commit('purchaseWithToken', post) :
      ...
      def authorize(money, payment, options={})
        ...
        response = (payment.is_a?(String) ?
                    commit('preAuthorizationWithToken', post) :
      ...
      def capture(money, authorization, options={})
        ...
        add_money(post, money)
        add_customer_data(post, options)
        transaction_number, authorization_number, invoice_number =
      ...
      def refund(money, authorization, options={})
        ...
        add_money(post, money)
        add_customer_data(post, options)
        transaction_number, _, invoice_number =
          split_authorization(authorization)
        post[:OriginalTransactionNumber] = transaction_number
        post[:OriginalInvoiceNumber] = invoice_number
      ...
      def credit(money, payment, options={})
        ...
        response = (payment.is_a?(String)? commit('refundWithToken', post) :
                    commit('refund', post))
        send_ack(response.authorization)
        response
      end
      ...
      def add_address(post, creditcard, options)
        if address = options[:billing_address] || options[:address]
          post[:CardHolderAddress] = ("#{address[:address1]}
                                      #{address[:address2]}").rjust(20, ' ')
      ...
      def post_data(action, parameters = {})
        ...
        parameters = parameters.collect do |key, value|
          "#{key}=#{value.to_s}" unless (value.nil? || value.empty?)
        end.join('&')
      ...
      def error_code_from(response)
        unless success_from(response)
          STANDARD_ERROR_CODE_MAPPING[response['returnCode'].strip ||
                                      response['recurReturnCode'.strip]]
        end
      end
    end
  end
